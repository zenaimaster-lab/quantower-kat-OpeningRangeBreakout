...
private:
   ...
   int    m_winsToday;
   int    m_lossesToday;
   // Per-timeframe W/L (0=2m, 1=5m, 2=15m, 3=30m)
   int    m_winsTF[4];
   int    m_lossesTF[4];
...
public:
   ...
   {
      for(int i=0; i<4; i++) { m_winsTF[i]=0; m_lossesTF[i]=0; }
   }
   ...
   // Per-timeframe: idx 0=2m, 1=5m, 2=15m, 3=30m
   int    WinsTodayTF(int idx) const     { return (idx>=0 && idx<4) ? m_winsTF[idx] : 0; }
   void   SetWinsTodayTF(int idx, int v) { if(idx>=0 && idx<4) m_winsTF[idx] = v; }
   int    LossesTodayTF(int idx) const     { return (idx>=0 && idx<4) ? m_lossesTF[idx] : 0; }
   void   SetLossesTodayTF(int idx, int v) { if(idx>=0 && idx<4) m_lossesTF[idx] = v; }
};
